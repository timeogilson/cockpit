  ...
  return (
    <section className="flex h-full min-w-[230px] flex-1 flex-col">
      <header className="mb-2 flex items-center gap-2 px-0.5">
        ...
        <h2 className="text-[12px] font-semibold uppercase tracking-wide text-ink-100/70">
          {meta.label}
        </h2>
        <span className="rounded-full bg-ink-800 px-1.5 text-[11px] font-medium text-ink-500">
          {agents.length}
        </span>
      </header>
      ...
      <div className="flex flex-1 flex-col gap-2 overflow-y-auto pr-1">
        {agents.length === 0 ? (
          <div className="grid flex-1 place-items-center rounded-lg border border-dashed border-ink-800 px-3 py-8 text-center text-[11.5px] text-ink-600">
            {EMPTY_COPY[status]}
          </div>
        ) : (
