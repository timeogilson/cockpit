    ...
    <aside className="flex w-64 shrink-0 flex-col border-l border-ink-700/70 bg-ink-900 shadow-rail">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4">
        ...
        {a && a.cache.cacheReadTokens > 0 && (
          <section className="mt-3 rounded-md border border-ink-800 bg-ink-850 px-2.5 py-2">
            <div className="flex items-center justify-between text-[11px]">
              <span className="uppercase tracking-wide text-ink-600">Cache hit</span>
              <span className="font-mono text-status-done">{formatPct(a.cache.efficiency)}</span>
            </div>
            <p className="mt-0.5 text-[10.5px] text-ink-500">
              saved <span className="font-mono text-ink-100/70">{formatCost(a.cache.savedUsd)}</span>
            </p>
          </section>
        )}
        ...
        <section className="mt-5">
          ...
          {models.length > 0 ? (
            ...
            <p className="text-[11.5px] text-ink-600">No spend recorded yet.</p>
          )}
        </section>
...
function WindowBar({ label, win }: { label: string; win: UsageWindow }): JSX.Element {
  ...
  const barColor = over ? 'bg-status-failed' : near ? 'bg-status-input' : 'bg-status-busy/80';
  return (
    <div>
      <div className="flex items-center justify-between text-[11.5px]">
        <span className="text-ink-100/75">{label}</span>
        <span className="font-mono text-ink-400">
          {formatCost(win.costUsd)}
          {win.estimated && <span className="ml-0.5 text-ink-600">~</span>}
          {pct !== undefined && (
            <span className={`ml-1 ${over ? 'text-status-failed' : 'text-ink-600'}`}>
              {formatPct(pct)}
            </span>
          )}
        </span>
...
function Stat({
  ...
  return (
    <div className="rounded-md border border-ink-800 bg-ink-850 px-2.5 py-1.5">
      ...
      <p className={`font-mono text-[13px] ${accent ? 'text-accent-soft' : 'text-ink-100/85'}`}>
        {value}
      </p>
    </div>
  );
