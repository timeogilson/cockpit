    ...
    <li>
      ...
      >
        <button
          ...
          className={`text-ink-600 ${hasChildren ? 'cursor-pointer' : 'cursor-default opacity-0'}`}
        ...
        <span className="rounded border border-ink-700 bg-ink-800 px-1.5 py-0.5 text-[10px] text-ink-100/60">
          {modelLabel(node.model)}
        </span>
        <span className="w-14 text-right font-mono text-[11px] text-ink-400">{formatCost(node.costUsd)}</span>
      </div>
      <div className="ml-7 flex flex-wrap gap-x-3 gap-y-0.5 pb-1 text-[10.5px] text-ink-600" style={{ marginLeft: depth * 12 + 28 }}>
  ...
  return (
    <div>
      <p className="mb-1.5 text-[11px] text-ink-500">
        ...
        <span className="font-mono">{formatCost(totalCost)}</span> combined
      </p>
